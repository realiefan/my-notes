  ...
  const clearInputs = () => {
   ...
   return (
    <div className="min-h-screen flex flex-col items-center bg-gray-900">
      <div className="w-screen rounded-lg p-4 bg-gray-900 text-white">
        ...
        <textarea
          ...
          className="w-full p-2 h-32 border rounded focus:outline-none focus:border-blue-500 bg-gray-800 text-white"
        />
        <div className="flex justify-center items-center mt-4">
          <div>
            <button onClick={saveNote} className="px-2 py-1 rounded-md text-md font-semibold text-black bg-gray-300">
              Save Note
            </button>
          </div>
        </div>
        ...
        <input
          ...
          className="w-full p-1 mt-4 border rounded text-sm focus:outline-none focus:border-blue-500 bg-gray-800 text-white"
        />
       ...
       <ul id="notesList" className="mt-4 grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {searchResults
          .sort((a, b) => b.id - a.id) 
          .map((note) => (
            <li key={note.id} className="bg-gray-800 p-4 rounded shadow">
              <h3 className="text-blue-400 text-s font-semibold mb-2">{note.title}</h3>
              <p className="text-gray-300 mb-2">{note.content}</p>
              <div className="flex justify-end">
                <button onClick={() => editNote(note.id)} className="text-blue-400 mr-2">
                  Edit
                </button>
